\documentclass[a4paper, fontsize=8pt, landscape]{scrartcl}

\input{header}
\graphicspath{{images/}}

\title{System Security}
\author{Theo von Arx}
\date{\today}

\begin{document}
\setcounter{secnumdepth}{2} %no enumeration of sections
\begin{multicols*}{3}
	\maketitle

	\section*{Imprint}
	\subsection*{Authors}
	This summary was written by
	\begin{itemize}
		\item \href{mailto:varxth@student.ethz.ch}{Theo von Arx}.
		\item \href{mailto:tom.cinbis@student.ethz.ch}{Tom Cinbis}.
	\end{itemize}
	It is based on
	\begin{itemize}
	  \item the lecture slides \textit{System Security} (Fall
	    2019, ETH Zurich) by Srdjan Čapkun,
	    Adrian Perrig, and Kari Kostiainen;
	  \item various papers.
	\end{itemize}
	\subsection*{Source Code}
	% TODO
	\subsection*{License}
	\doclicenseLongText
	\begin{center}\doclicenseImage[imagewidth=0.5\columnwidth]\end{center}

	\bigskip

	\input{chapters/basics.tex}
	\input{chapters/owasp.tex}
	\input{chapters/sideChannelAttacks.tex}
	\input{chapters/tamperResilience.tex}
	\input{chapters/commodity_systems.tex}
	\input{chapters/sgx.tex}
	\input{chapters/meltdown_spectre.tex}
	\input{chapters/os_security.tex}
	\input{chapters/mobile_platform_security.tex}
	\input{chapters/tcg.tex}
	\input{chapters/software_attestation.tex}
	\input{chapters/iot.tex}
	\input{chapters/stack_attacks.tex}
\end{multicols*}

	\input{chapters/comparison_sgx_trustzone_tcg.tex}
\setcounter{secnumdepth}{2}
\end{document}

